...
import { OpenLoginWebSDK } from '@gooddollar/web3sdk-v2'
...
function web3auth(options: Web3AuthModuleOptions): WalletInit {
    return () => ({
        ...
        getInterface: async ({ EventEmitter, chains }) => {
            ...
            const { createEIP1193Provider, ProviderRpcError, ProviderRpcErrorCode } = await import(
                '@web3-onboard/common'
            )
          ...
          let web3auth = new OpenLoginWebSDK()
          await web3auth.initialize(web3authCoreOptions)
          // let web3auth = await web3authNew.initialize(web3authCoreOptions)
            ...
            // const subscribeAuthEvents = (web3auth: OpenLoginWebSDK) => {
          ...
          let web3AuthProvider = await web3auth.login()
            ...
            function patchProvider(): EIP1193Provider {
                const patchedProvider = createEIP1193Provider(web3AuthProvider, {
                    ...
                    wallet_switchEthereumChain: async ({ params }) => {
                        ...
                        // re-instantiate instance with new network -- not sure what should happen here?
                        let web3auth = new OpenLoginWebSDK()
                        await web3auth.initialize(web3authCoreOptions)

                        // await web3auth.initModal(modalConfig)

                        web3AuthProvider = await web3auth.login()
